...
def pong_app(container):
    ...
    ball_dx, ball_dy = 3, -3
    score = 0
    score_text = canvas.create_text(190, 15, text="Points: 0", fill="white", font=("Courier", 12))
    ...
    def game_loop():
        ...
        if by2 >= py1 and px1 < (bx1+bx2)/2 < px2 and ball_dy > 0:
            ...
            ball_dy *= 1.05
        if by2 >= 250:
            canvas.create_text(190, 125, text="PARTIE TERMINÉ", fill="white", font=("Courier", 14))
            return
...
def settings_app(container):
    ...
    # Title
    tk.Label(scroll_frame, text="Réglages", font=("Segoe UI", 14, "bold"),
             bg="#e0e0e0").pack(pady=10)

    # Wallpaper Color Section
    tk.Label(scroll_frame, text="Couleur de L'arrière-plan:", bg="#e0e0e0",
    ...
    tk.Label(scroll_frame, text="Couleur de la barre des tâches:", bg="#e0e0e0",
    ...
    tk.Button(scroll_frame, text="Réinitialiser par Défaut", bg="#d0d0d0",
              command=reset_defaults).pack(pady=10)

# ---------------- App Store App ----------------
def app_store_app(container):
    container.configure(bg="#f0f0f0")
    tk.Label(container, text="Achats LBOS", font=("Segoe UI", 14, "bold"),
             bg="#f0f0f0").pack(pady=10)

    app_frame = tk.Frame(container, bg="#ffffff", bd=1, relief="solid")
    app_frame.pack(pady=5, padx=10, fill="x")
    tk.Label(app_frame, text="🎮 Le Jeu Pong", font=("Segoe UI", 12, "bold"),
             bg="#ffffff").pack(anchor="w", padx=5, pady=2)
    tk.Label(app_frame, text="Le jeu original de LB65memes. Gratuit!", bg="#ffffff").pack(anchor="w", padx=5)

    status_label = tk.Label(app_frame, text="", bg="#ffffff", fg="green")
    status_label.pack()

    def add_pong_icon():
        status_label.config(text="Installé!")
        tk.Button(
            desktop,
            text="🎮 Pong",
            font=("Segoe UI", 14),
    ...
    tk.Button(app_frame, text="Installer", command=install_pong).pack(pady=5)
...
tk.Button(
    ...
    text="🏬 Achats",
    font=("Segoe UI", 14),
    command=lambda: make_app_window("Achats", app_store_app, 350, 300, 120, 120)
).place(x=50, y=50)
...
tk.Button(
    ...
    text="⚙️ Réglages",
    font=("Segoe UI", 14),
    command=lambda: make_app_window("Réglages", settings_app, 653, 300, 150, 150)
).place(x=250, y=50)
